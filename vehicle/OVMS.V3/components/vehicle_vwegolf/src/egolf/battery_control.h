...
// [FW+WIRE] confirmed by a 10-point live sweep (emanager-profiles.crtd):
...
// can match the reply. [FW BatteryControlProfilesAH header; WIRE rolling 0x11..0x1f
// in emanager-profiles.crtd.]  NB this is the COMMAND-side header; the FSG STATUS
// broadcast uses a different 5-byte header (see decodeProfileArray).
constexpr uint8_t kAsgIdMib = 0x1;   // MIB2 display
constexpr uint8_t kAsgIdOcu = 0x2;   // OCU / remote client (an OVMS emulates this)
